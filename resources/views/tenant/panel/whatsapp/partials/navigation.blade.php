<nav class="rounded-2xl border border-stone-200 bg-white/95 px-4 py-3 shadow-[0_12px_30px_-24px_rgba(15,23,42,0.35)] backdrop-blur">
    <div class="flex flex-wrap items-center gap-2">
        @if (($navigation['can_view_relationship'] ?? false) === true)
            <a
                href="{{ $navigation['relationship_url'] }}"
                class="inline-flex items-center rounded-full px-3 py-1.5 text-sm font-semibold transition {{ ($navigation['active'] ?? null) === 'relationship' ? 'bg-slate-950 text-white' : 'border border-stone-300 bg-white text-slate-700 hover:bg-stone-50' }}"
            >
                Relacionamento
            </a>
        @endif
